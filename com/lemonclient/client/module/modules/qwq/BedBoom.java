...
@Module.Declaration(name = "BedBoom", category = Category.qwq, priority = 999)
public class BedBoom extends Module
{
    ModeSetting page;
    ...
    public void onUpdate() {
        if (BedBoom.mc.player == null || BedBoom.mc.world == null || BedBoom.mc.player.isDead || this.inNether()) {
            return;
        }
    ...
    private void check() {
        this.burrowed = false;
        final BlockPos originalPos = new BlockPos(BedBoom.mc.player.posX, BedBoom.mc.player.posY, BedBoom.mc.player.posZ);
        if (BedBoom.mc.world.getBlockState(originalPos).getBlock() == Blocks.BEDROCK || BedBoom.mc.world.getBlockState(originalPos).getBlock() == Blocks.OBSIDIAN || BedBoom.mc.world.getBlockState(originalPos).getBlock() == Blocks.ENDER_CHEST) {
            this.burrowed = true;
        }
    }
    
    private boolean inNether() {
        return BedBoom.mc.player.dimension == 0;
    }
    ...
    private void calc() {
        if (this.calctiming.passedMs((long)(int)this.calcdelay.getValue())) {
            ...
            if (this.renderEntity == null) {
                ...
                for (final Entity entity : BedBoom.mc.world.loadedEntityList) {
                    if (BedBoom.mc.player.getDistance(entity) <= (int)this.enemyRange.getValue()) {
                        if (entity.isDead) {
                            continue;
                        }
            ...
            if (!((String)this.handmode.getValue()).equals("Main") && BedBoom.mc.player.getHeldItemOffhand().getItem() == Items.BED) {
                offhand = true;
            }
            if (!offhand && !((String)this.handmode.getValue()).equals("Off")) {
                this.oldslot = BedBoom.mc.player.inventory.currentItem;
            ...
            if ((boolean)this.base.getValue() && this.shouldbaseplace) {
                ...
                final int oldSlot = BedBoom.mc.player.inventory.currentItem;
            ...
            this.SelfDamage = DamageUtil.calcDamage(bedpos.getX() + 0.5, bedpos.getY() + 1.5626, bedpos.getZ() + 0.5626, (Entity)BedBoom.mc.player);
            if (this.inRange(bedpos.north().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.SOUTH)) {
                this.face = "SOUTH";
                bedpos = new BlockPos(bedpos.x, bedpos.y + 1, bedpos.z - 1);
            }
            else if (this.inRange(bedpos.east().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.WEST)) {
                this.face = "WEST";
                bedpos = new BlockPos(bedpos.x + 1, bedpos.y + 1, bedpos.z);
            }
            else if (this.inRange(bedpos.south().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.NORTH)) {
                this.face = "NORTH";
                bedpos = new BlockPos(bedpos.x, bedpos.y + 1, bedpos.z + 1);
            }
            ...
            if ((BedBoom.mc.world.isAirBlock(bedpos.down()) || (!BedBoom.mc.world.isAirBlock(bedpos) && BedBoom.mc.world.getBlockState(bedpos).getBlock() != Blocks.BED)) && (boolean)this.autorotate.getValue() && (!(boolean)this.pause1.getValue() || !this.burrowed)) {
                bedpos = pos;
                if (this.inRange(bedpos.east().up()) && this.block(bedpos.east()) && (BedBoom.mc.world.isAirBlock(bedpos.east().up()) || BedBoom.mc.world.getBlockState(bedpos.east().up()).getBlock() == Blocks.BED)) {
                    this.face = "WEST";
                    this.rotation = new Vec2f(90.0f, BedBoom.mc.player.rotationPitch);
                    bedpos = new BlockPos(bedpos.x + 1, bedpos.y + 1, bedpos.z);
                }
                else if (this.inRange(bedpos.north().up()) && this.block(bedpos.north()) && (BedBoom.mc.world.isAirBlock(bedpos.north().up()) || BedBoom.mc.world.getBlockState(bedpos.north().up()).getBlock() == Blocks.BED)) {
                    this.face = "SOUTH";
                    this.rotation = new Vec2f(0.0f, BedBoom.mc.player.rotationPitch);
                    bedpos = new BlockPos(bedpos.x, bedpos.y + 1, bedpos.z - 1);
                }
                else if (this.inRange(bedpos.west().up()) && this.block(bedpos.west()) && (BedBoom.mc.world.isAirBlock(bedpos.west().up()) || BedBoom.mc.world.getBlockState(bedpos.west().up()).getBlock() == Blocks.BED)) {
                    this.face = "EAST";
                    this.rotation = new Vec2f(-90.0f, BedBoom.mc.player.rotationPitch);
                    bedpos = new BlockPos(bedpos.x - 1, bedpos.y + 1, bedpos.z);
                }
                else {
                    if (!this.inRange(bedpos.south().up()) || !this.block(bedpos.south()) || (!BedBoom.mc.world.isAirBlock(bedpos.south().up()) && BedBoom.mc.world.getBlockState(bedpos.south().up()).getBlock() != Blocks.BED)) {
                        return;
                    }
                    this.face = "NORTH";
                    this.rotation = new Vec2f(180.0f, BedBoom.mc.player.rotationPitch);
                    bedpos = new BlockPos(bedpos.x, bedpos.y + 1, bedpos.z + 1);
                }
            }
            this.render2 = bedpos;
    ...
    private void bedaura() {
        ...
        if (BedBoom.mc.world.getBlockState(this.render).getBlock() == Blocks.BED || BedBoom.mc.world.getBlockState(this.render2).getBlock() == Blocks.BED) {
            this.Break();
        }
        else {
            this.place();
    ...
    private void Break() {
        if (this.breaktiming.passedMs((long)(int)this.breakdelay.getValue())) {
            ...
            if (BedBoom.mc.player.isSneaking()) {
                BedBoom.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)BedBoom.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
            }
            final Vec3d hitfacing = this.getHitVecOffset(side);
            BedBoom.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(this.render2, side, this.hand, (float)hitfacing.x, (float)hitfacing.y, (float)hitfacing.z));
    ...
    private void place() {
        if (this.placetiming.passedMs((long)(int)this.placedelay.getValue())) {
            if ((boolean)this.rendertest.getValue() || this.render2 == null || !BedBoom.mc.world.isAirBlock(this.render2)) {
                return;
            }
            ...
            boolean sneak = false;
            if (BlockUtil.blackList.contains(BedBoom.mc.world.getBlockState(neighbour).getBlock()) && !BedBoom.mc.player.isSneaking()) {
                BedBoom.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)BedBoom.mc.player, CPacketEntityAction.Action.START_SNEAKING));
                sneak = true;
            }
    ...
    private void switchTo(final int slot) {
        if (slot > -1 && slot < 9 && (!(boolean)this.check.getValue() || BedBoom.mc.player.inventory.currentItem != slot)) {
            if (this.packetSwitch.getValue()) {
                BedBoom.mc.player.connection.sendPacket((Packet)new CPacketHeldItemChange(slot));
            }
            else {
                BedBoom.mc.player.inventory.currentItem = slot;
            }
            BedBoom.mc.playerController.updateController();
        }
    }
    ...
    private List<EntityPlayer> getTargetList(final double range) {
        return (List<EntityPlayer>)BedBoom.mc.world.playerEntities.stream().filter(p -> BedBoom.mc.player.getDistance(p) <= range).filter(p -> BedBoom.mc.player.entityId != p.entityId).filter(p -> !EntityUtil.basicChecksEntity(p)).collect(Collectors.toList());
    }
    
    public static EntityPlayer getNearestPlayer(final List<EntityPlayer> list) {
        return list.stream().min(Comparator.comparing(p -> BedBoom.mc.player.getDistance(p))).orElse(null);
    }
            ...
            case "Damage": {
                ...
                for (final EntityPlayer player : this.getTargetList((int)this.enemyRange.getValue())) {
                    ...
                    if (!player.isDead) {
                        ...
                        final EntityPlayer self = (EntityPlayer)(((boolean)this.predictself.getValue()) ? PredictUtil.predictPlayer((EntityPlayer)BedBoom.mc.player, this.settings) : BedBoom.mc.player);
                ...
                if (list.isEmpty()) {
                    ...
                    for (final EntityPlayer entityPlayer : list.keySet()) {
                        final double range = BedBoom.mc.player.getDistanceSq((BlockPos)list.get(entityPlayer));
                        if (player2 == null) {
    ...
    private boolean block(final BlockPos pos) {
        return BedBoom.mc.world.getBlockState(pos).getBlock() != Blocks.BED && BedBoom.mc.world.getBlockState(pos).getBlock() != Blocks.TRIPWIRE && (BedBoom.mc.world.getBlockState(pos.up()).getBlock() == Blocks.BED || (BedBoom.mc.world.isAirBlock(pos.up()) && ((boolean)this.highVersion.getValue() || BedBoom.mc.world.getBlockState(pos).isSideSolid((IBlockAccess)BedBoom.mc.world, pos, EnumFacing.UP))));
    }
    ...
    private boolean canPlaceBed(final BlockPos blockPos) {
        ...
        if ((boolean)this.autorotate.getValue() && (!(boolean)this.pause1.getValue() || !this.burrowed)) {
            return (this.inRange(blockPos.east().up()) && this.block(blockPos.east()) && (BedBoom.mc.world.isAirBlock(blockPos.east().up()) || BedBoom.mc.world.getBlockState(blockPos.east().up()).getBlock() == Blocks.BED)) || (this.inRange(blockPos.north().up()) && this.block(blockPos.north()) && (BedBoom.mc.world.isAirBlock(blockPos.north().up()) || BedBoom.mc.world.getBlockState(blockPos.north().up()).getBlock() == Blocks.BED)) || (this.inRange(blockPos.west().up()) && this.block(blockPos.west()) && (BedBoom.mc.world.isAirBlock(blockPos.west().up()) || BedBoom.mc.world.getBlockState(blockPos.west().up()).getBlock() == Blocks.BED)) || (this.inRange(blockPos.south().up()) && this.block(blockPos.south()) && BedBoom.mc.world.isAirBlock(blockPos.south().up())) || BedBoom.mc.world.getBlockState(blockPos.south().up()).getBlock() == Blocks.BED;
        }
        BlockPos pos;
        if (BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.SOUTH)) {
            pos = new BlockPos(blockPos.x, blockPos.y, blockPos.z - 1);
        }
        else if (BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.WEST)) {
            pos = new BlockPos(blockPos.x + 1, blockPos.y, blockPos.z);
        }
        else if (BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.NORTH)) {
            pos = new BlockPos(blockPos.x, blockPos.y, blockPos.z + 1);
        }
        else {
            pos = new BlockPos(blockPos.x - 1, blockPos.y, blockPos.z);
        }
        return this.block(pos) && (BedBoom.mc.world.isAirBlock(pos.up()) || BedBoom.mc.world.getBlockState(pos.up()).getBlock() == Blocks.BED) && this.inRange(pos.up());
    }
    ...
    private boolean canPlaceBedWithoutBase(final BlockPos blockPos) {
        ...
        if ((boolean)this.autorotate.getValue() && (!(boolean)this.pause1.getValue() || !this.burrowed)) {
            return (this.inRange(blockPos.east().up()) && (this.block(blockPos.east()) || BedBoom.mc.world.isAirBlock(blockPos.east().up()))) || (this.inRange(blockPos.north().up()) && (this.block(blockPos.north()) || BedBoom.mc.world.isAirBlock(blockPos.north().up()))) || (this.inRange(blockPos.west().up()) && (this.block(blockPos.west()) || BedBoom.mc.world.isAirBlock(blockPos.west().up()))) || (this.inRange(blockPos.south().up()) && (this.block(blockPos.south()) || BedBoom.mc.world.isAirBlock(blockPos.south().up())));
        }
        BlockPos pos;
        if (this.inRange(blockPos.north().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.SOUTH)) {
            pos = new BlockPos(blockPos.x, blockPos.y, blockPos.z - 1);
        }
        else if (this.inRange(blockPos.east().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.WEST)) {
            pos = new BlockPos(blockPos.x + 1, blockPos.y, blockPos.z);
        }
        else if (this.inRange(blockPos.south().up()) && BedBoom.mc.player.getHorizontalFacing().equals((Object)EnumFacing.NORTH)) {
            pos = new BlockPos(blockPos.x, blockPos.y, blockPos.z + 1);
        }
        else {
            pos = new BlockPos(blockPos.x - 1, blockPos.y, blockPos.z);
        }
        return this.inRange(pos.up()) && (this.block(pos) || BedBoom.mc.world.isAirBlock(pos.up()));
    }
    ...
    private boolean canPlace(final BlockPos basePos, final BlockPos headPos) {
        ...
        return this.inRange(basePos.up()) && this.isPos2(basePos, pos) && (this.block(pos) || BedBoom.mc.world.isAirBlock(pos.up()));
    }
    ...
    private BlockPos getBestBasePos(final BlockPos pos) {
        ...
        for (final BlockPos side : this.sides) {
            ...
            if (this.inRange(base)) {
                if ((BedBoom.mc.world.isAirBlock(base.up()) || BedBoom.mc.world.getBlockState(base.up()).getBlock() == Blocks.BED) && this.inRange(base.add(0, 1, 0)) && !this.intersectsWithEntity(base)) {
                    if (this.canPlace(base, pos)) {
                        return base;
                    }
                    if (bestPos == null) {
                        bestRange = BedBoom.mc.player.getDistanceSq(base);
                        bestPos = base;
                    }
                    else if (bestRange > BedBoom.mc.player.getDistanceSq(base)) {
                        bestRange = BedBoom.mc.player.getDistanceSq(base);
                        bestPos = base;
                    }
                }
    ...
    private boolean inRange(final BlockPos pos) {
        ...
        return BedBoom.mc.player.getDistanceSq(pos) <= (double)this.Range.getValue() * (double)this.Range.getValue() && y * y <= (double)this.yRange.getValue() * (double)this.yRange.getValue();
    }
    ...
    public BlockPos calculateBestPlacement(final EntityPlayer player) {
        ...
        if (this.predictself.getValue()) {
            self = PredictUtil.predictPlayer((EntityPlayer)BedBoom.mc.player, this.settings);
        }
        else {
            self = (EntityPlayer)BedBoom.mc.player;
        }
        final double Hp = self.getHealth() + self.getAbsorptionAmount();
        BlockPos best = null;
        float bestDamage = 0.0f;
    ...
    public void refill_bed() {
        if (!(BedBoom.mc.currentScreen instanceof GuiContainer) || BedBoom.mc.currentScreen instanceof GuiInventory) {
            final int airslot = this.is_space();
            if (airslot != -1) {
                int i = 9;
                while (i < 36) {
                    if (BedBoom.mc.player.inventory.getStackInSlot(i).getItem() == Items.BED) {
                        if (((String)this.clickmode.getValue()).equalsIgnoreCase("Quick")) {
                            BedBoom.mc.playerController.windowClick(BedBoom.mc.player.inventoryContainer.windowId, i, 0, ClickType.QUICK_MOVE, (EntityPlayer)BedBoom.mc.player);
                            BedBoom.mc.playerController.updateController();
                            break;
                        }
                        if (((String)this.clickmode.getValue()).equalsIgnoreCase("Swap")) {
                            BedBoom.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)BedBoom.mc.player);
                            BedBoom.mc.playerController.windowClick(0, airslot + 36, 0, ClickType.SWAP, (EntityPlayer)BedBoom.mc.player);
                            BedBoom.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)BedBoom.mc.player);
                            BedBoom.mc.playerController.updateController();
                            break;
                        }
                        BedBoom.mc.playerController.windowClick(BedBoom.mc.player.inventoryContainer.windowId, i, 0, ClickType.PICKUP, (EntityPlayer)BedBoom.mc.player);
                        BedBoom.mc.playerController.windowClick(BedBoom.mc.player.inventoryContainer.windowId, airslot + 36, 0, ClickType.PICKUP, (EntityPlayer)BedBoom.mc.player);
                        BedBoom.mc.playerController.updateController();
                        break;
                    }
                    else {
    ...
    private int is_space() {
        ...
        if (this.force.getValue()) {
            if (((String)this.refillmode.getValue()).equals("Only")) {
                ...
                if (BedBoom.mc.player.inventory.getStackInSlot(slot2).getItem() != Items.BED) {
                    slot = slot2;
                }
            }
            else {
            ...
            final int slot2 = (int)this.slotS.getValue() - 1;
            if (BedBoom.mc.player.inventory.getStackInSlot(slot2).getItem() == Items.AIR) {
                slot = slot2;
            }
        }
        else {
            for (int i = 0; i < 9; ++i) {
                if (BedBoom.mc.player.inventory.getStackInSlot(i).getItem() == Items.AIR) {
                    slot = i;
                }
            }
        }
    ...
    private void swing(final EnumHand hand) {
        if (this.packetswing.getValue()) {
            BedBoom.mc.player.connection.sendPacket((Packet)new CPacketAnimation(hand));
        }
        else {
            BedBoom.mc.player.swingArm(hand);
        }
    }
    ...
    class renderBlock
    {
        ...
        void render() {
            BedBoom.this.drawBoxMain(this.pos, this.pos2, this.returnGradient(), this.returnOutlineGradient());
        }
        
        public int returnGradient() {
            final long end = this.start + (int)BedBoom.this.lifeTime.getValue();
            int result = (int)((end - System.currentTimeMillis()) / (float)(end - this.start) * 100.0f);
            if (result < 0) {
                result = 0;
            }
            final int startFade = (int)BedBoom.this.fadealpha.getValue();
            final int endFade = 0;
    ...
    class managerClassRenderBlocks
    {
        ...
        void render() {
            this.blocks.forEach(e -> {
                if (BedBoom.this.render != null && ((BedBoom.this.isPos2(e.pos, BedBoom.this.render) && BedBoom.this.isPos2(e.pos2, BedBoom.this.render2)) || (BedBoom.this.isPos2(e.pos2, BedBoom.this.render) && BedBoom.this.isPos2(e.pos, BedBoom.this.render2)))) {
                    e.resetTime();
                }
                else {
                    e.render();
        ...
        void addRender(final BlockPos pos, final BlockPos pos2) {
            ...
            for (final renderBlock block : this.blocks) {
                if ((BedBoom.this.isPos2(block.pos, pos) && BedBoom.this.isPos2(block.pos2, pos2)) || (BedBoom.this.isPos2(block.pos2, pos2) && BedBoom.this.isPos2(block.pos, pos))) {
                    render = false;
                    block.resetTime();
                    break;
                }

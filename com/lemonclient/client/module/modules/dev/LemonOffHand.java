...
@Module.Declaration(name = "LemonOffhand", category = Category.Dev)
public class LemonOffHand extends Module
{
    ...
    public void onUpdate() {
        if (LemonOffHand.mc.currentScreen instanceof GuiContainer && !(LemonOffHand.mc.currentScreen instanceof GuiInventory)) {
            return;
        }
        if ((boolean)this.hotBarTotem.getValue() && (boolean)this.refill.getValue()) {
            boolean hasTotem = false;
            for (int i = 0; i < 9; ++i) {
                if (LemonOffHand.mc.player.inventory.getStackInSlot(i).getItem() == Items.TOTEM_OF_UNDYING) {
                    hasTotem = true;
                }
            }
                ...
                while (i < 36) {
                    if (LemonOffHand.mc.player.inventory.getStackInSlot(i).getItem() == Items.TOTEM_OF_UNDYING) {
                        if ((int)this.totemSlot.getValue() == 1) {
                            LemonOffHand.mc.playerController.windowClick(LemonOffHand.mc.player.inventoryContainer.windowId, i, 0, ClickType.SWAP, (EntityPlayer)LemonOffHand.mc.player);
                            break;
                        }
                        LemonOffHand.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)LemonOffHand.mc.player);
                        LemonOffHand.mc.playerController.windowClick(0, (int)this.totemSlot.getValue() + 35, 0, ClickType.SWAP, (EntityPlayer)LemonOffHand.mc.player);
                        LemonOffHand.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)LemonOffHand.mc.player);
                        break;
                    }
                    else {
        ...
        if (this.stepChanging) {
            ...
            LemonOffHand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, (EntityPlayer)LemonOffHand.mc.player);
            this.switchDone.add(System.currentTimeMillis());
        }
        this.totems = LemonOffHand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
        if (this.returnBack) {
            if (this.tickWaited++ < (int)this.tickDelay.getValue()) {
                return;
            }
    ...
    private void GetOffhand() {
        if (((String)this.HudMode.getValue()).equals("Offhand")) {
            this.item = LemonOffHand.mc.player.getHeldItemOffhand().getItem();
            final int items = LemonOffHand.mc.player.getHeldItemOffhand().getCount();
            this.Itemname = LemonOffHand.mc.player.getHeldItemOffhand().getDisplayName();
            this.counts = LemonOffHand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == this.item).mapToInt(ItemStack::getCount).sum() + items;
        }
    }
    
    private void changeBack() {
        if (this.prevSlot == -1 || !LemonOffHand.mc.player.inventory.getStackInSlot(this.prevSlot).isEmpty()) {
            this.prevSlot = this.findEmptySlot();
        }
        if (this.prevSlot != -1) {
            LemonOffHand.mc.playerController.windowClick(0, (this.prevSlot < 9) ? (this.prevSlot + 36) : this.prevSlot, 0, ClickType.PICKUP, (EntityPlayer)LemonOffHand.mc.player);
        }
        else {
    ...
    private boolean switchItemTotemHot() {
        ...
        if (slot != -1) {
            if (LemonOffHand.mc.player.inventory.currentItem != slot) {
                LemonOffHand.mc.player.inventory.currentItem = slot;
            }
            return true;
        }
        return false;
    ...
    private String getItem() {
        ...
        if (((boolean)this.fallDistanceBol.getValue() && LemonOffHand.mc.player.fallDistance >= (int)this.fallDistance.getValue() && LemonOffHand.mc.player.prevPosY != LemonOffHand.mc.player.posY && !LemonOffHand.mc.player.isElytraFlying()) || ((boolean)this.crystalCheck.getValue() && this.crystalDamage())) {
            normalOffHand = false;
            itemCheck = "Totem";
        }
        final Item mainHandItem = LemonOffHand.mc.player.getHeldItemMainhand().getItem();
        if (mainHandItem instanceof ItemSword) {
            boolean can = true;
            if (LemonOffHand.mc.gameSettings.keyBindUseItem.isKeyDown() && (boolean)this.swordCheck.getValue()) {
                if ((boolean)this.shiftPot.getValue() && LemonOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
                    can = false;
                    itemCheck = "Pot";
                    normalOffHand = false;
                }
        ...
        if (mainHandItem == Items.DIAMOND_PICKAXE) {
            if (!LemonOffHand.mc.gameSettings.keyBindSneak.isKeyDown() || LemonOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
            ...
            if (LemonOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
        ...
        if (normalOffHand && (boolean)this.antiWeakness.getValue() && LemonOffHand.mc.player.isPotionActive(MobEffects.WEAKNESS)) {
            normalOffHand = false;
            itemCheck = "Crystal";
        }
    ...
    private boolean nearPlayer() {
        ...
        for (final EntityPlayer pl : LemonOffHand.mc.world.playerEntities) {
            if (pl != LemonOffHand.mc.player && LemonOffHand.mc.player.getDistance((Entity)pl) < (double)this.playerDistance.getValue()) {
                return true;
            }
        }
        return false;
    ...
    private boolean crystalDamage() {
        for (final Entity t : LemonOffHand.mc.world.loadedEntityList) {
            if (t instanceof EntityEnderCrystal && LemonOffHand.mc.player.getDistance(t) <= 12.0f && DamageUtil.calculateDamage(t.posX, t.posY, t.posZ, (Entity)LemonOffHand.mc.player) * (double)this.biasDamage.getValue() >= LemonOffHand.mc.player.getHealth() && this.totems > 0) {
                return true;
            }
        }
        return false;
    ...
    private int findEmptySlot() {
        for (int i = 35; i > -1; --i) {
            if (LemonOffHand.mc.player.inventory.getStackInSlot(i).isEmpty()) {
                return i;
            }
        }
        return -1;
    ...
    private boolean offHandSame(final String itemCheck) {
        final Item offHandItem = LemonOffHand.mc.player.getHeldItemOffhand().getItem();
    ...
    private String getItemToCheck(final String str) {
        if (LemonOffHand.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword) {
    ...
    private int isCorrect(final int i, final boolean blockBool, final Object item, final String itemName) {
        final Item temp = LemonOffHand.mc.player.inventory.getStackInSlot(i).getItem();
        if (blockBool) {
            ...
            if (itemName.equals("Pot") && !((String)this.potionChoose.getValue()).equalsIgnoreCase("first") && !LemonOffHand.mc.player.inventory.getStackInSlot(i).stackTagCompound.toString().split(":")[2].contains((CharSequence)this.potionChoose.getValue())) {
                return -1;
            }
            return i;
        }
    ...
    private void toOffHand(final int t) {
        if (!LemonOffHand.mc.player.getHeldItemOffhand().isEmpty()) {
            if (this.firstChange) {
                this.prevSlot = t;
            }
        ...
        LemonOffHand.mc.playerController.windowClick(0, (t < 9) ? (t + 36) : t, 0, ClickType.PICKUP, (EntityPlayer)LemonOffHand.mc.player);
    ...
    public String getHudInfo() {
        if (((String)this.HudMode.getValue()).equals("Totem")) {
            ...
            if (LemonOffHand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) {
                ++this.counts;
            }

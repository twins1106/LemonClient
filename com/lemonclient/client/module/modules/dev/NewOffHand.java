...
@Module.Declaration(name = "LemonOffhand2", category = Category.Dev)
public class NewOffHand extends Module
{
    ...
    public void onUpdate() {
        if (NewOffHand.mc.currentScreen instanceof GuiContainer && !(NewOffHand.mc.currentScreen instanceof GuiInventory)) {
            return;
        }
        if ((boolean)this.hotBarTotem.getValue() && (boolean)this.refill.getValue()) {
            boolean hasTotem = false;
            for (int i = 0; i < 9; ++i) {
                if (NewOffHand.mc.player.inventory.getStackInSlot(i).getItem() == Items.TOTEM_OF_UNDYING) {
                    hasTotem = true;
                }
            }
                ...
                while (i < 36) {
                    if (NewOffHand.mc.player.inventory.getStackInSlot(i).getItem() == Items.TOTEM_OF_UNDYING) {
                        if ((int)this.totemSlot.getValue() == 1) {
                            NewOffHand.mc.playerController.windowClick(NewOffHand.mc.player.inventoryContainer.windowId, i, 0, ClickType.SWAP, (EntityPlayer)NewOffHand.mc.player);
                            break;
                        }
                        NewOffHand.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)NewOffHand.mc.player);
                        NewOffHand.mc.playerController.windowClick(0, (int)this.totemSlot.getValue() + 35, 0, ClickType.SWAP, (EntityPlayer)NewOffHand.mc.player);
                        NewOffHand.mc.playerController.windowClick(0, i, 0, ClickType.SWAP, (EntityPlayer)NewOffHand.mc.player);
                        break;
                    }
                    else {
        ...
        if (this.stepChanging) {
            ...
            NewOffHand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, (EntityPlayer)NewOffHand.mc.player);
            this.switchDone.add(System.currentTimeMillis());
        }
        this.totems = NewOffHand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
        if (this.returnBack) {
            if (this.tickWaited++ < (int)this.tickDelay.getValue()) {
                return;
            }
    ...
    private void GetOffhand() {
        if (((String)this.HudMode.getValue()).equals("Offhand")) {
            this.item = NewOffHand.mc.player.getHeldItemOffhand().getItem();
            final int items = NewOffHand.mc.player.getHeldItemOffhand().getCount();
            this.Itemname = NewOffHand.mc.player.getHeldItemOffhand().getDisplayName();
            this.counts = NewOffHand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == this.item).mapToInt(ItemStack::getCount).sum() + items;
        }
    }
    
    private void changeBack() {
        if (this.prevSlot == -1 || !NewOffHand.mc.player.inventory.getStackInSlot(this.prevSlot).isEmpty()) {
            this.prevSlot = this.findEmptySlot();
        }
        if (this.prevSlot != -1) {
            NewOffHand.mc.playerController.windowClick(0, (this.prevSlot < 9) ? (this.prevSlot + 36) : this.prevSlot, 0, ClickType.PICKUP, (EntityPlayer)NewOffHand.mc.player);
        }
        else {
    ...
    private boolean switchItemTotemHot() {
        ...
        if (slot != -1) {
            if (NewOffHand.mc.player.inventory.currentItem != slot) {
                NewOffHand.mc.player.inventory.currentItem = slot;
            }
            return true;
        }
        return false;
    ...
    private String getItem() {
        ...
        if (((boolean)this.fallDistanceBol.getValue() && NewOffHand.mc.player.fallDistance >= (int)this.fallDistance.getValue() && NewOffHand.mc.player.prevPosY != NewOffHand.mc.player.posY && !NewOffHand.mc.player.isElytraFlying()) || ((boolean)this.crystalCheck.getValue() && this.crystalDamage())) {
            normalOffHand = false;
            itemCheck = "Totem";
        }
        final Item mainHandItem = NewOffHand.mc.player.getHeldItemMainhand().getItem();
        if (mainHandItem instanceof ItemSword) {
            boolean can = true;
            if (NewOffHand.mc.gameSettings.keyBindUseItem.isKeyDown() && (boolean)this.swordCheck.getValue()) {
                if ((boolean)this.shiftPot.getValue() && NewOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
                    can = false;
                    itemCheck = "Pot";
                    normalOffHand = false;
                }
        ...
        if (mainHandItem == Items.DIAMOND_PICKAXE) {
            if (!NewOffHand.mc.gameSettings.keyBindSneak.isKeyDown() || NewOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
            ...
            if (NewOffHand.mc.gameSettings.keyBindSneak.isKeyDown()) {
        ...
        if (normalOffHand && (boolean)this.antiWeakness.getValue() && NewOffHand.mc.player.isPotionActive(MobEffects.WEAKNESS)) {
            normalOffHand = false;
            itemCheck = "Crystal";
        }
    ...
    private boolean nearPlayer() {
        ...
        for (final EntityPlayer pl : NewOffHand.mc.world.playerEntities) {
            if (pl != NewOffHand.mc.player && NewOffHand.mc.player.getDistance((Entity)pl) < (double)this.playerDistance.getValue()) {
                return true;
            }
        }
        return false;
    ...
    private boolean crystalDamage() {
        for (final Entity t : NewOffHand.mc.world.loadedEntityList) {
            if (t instanceof EntityEnderCrystal && NewOffHand.mc.player.getDistance(t) <= 12.0f && DamageUtil.calculateDamage(t.posX, t.posY, t.posZ, (Entity)NewOffHand.mc.player) * (double)this.biasDamage.getValue() >= NewOffHand.mc.player.getHealth() && this.totems > 0) {
                return true;
            }
        }
        return false;
    ...
    private int findEmptySlot() {
        for (int i = 35; i > -1; --i) {
            if (NewOffHand.mc.player.inventory.getStackInSlot(i).isEmpty()) {
                return i;
            }
        }
        return -1;
    ...
    private boolean offHandSame(final String itemCheck) {
        final Item offHandItem = NewOffHand.mc.player.getHeldItemOffhand().getItem();
    ...
    private String getItemToCheck(final String str) {
        if (NewOffHand.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword) {
    ...
    private int isCorrect(final int i, final boolean blockBool, final Object item, final String itemName) {
        final Item temp = NewOffHand.mc.player.inventory.getStackInSlot(i).getItem();
        if (blockBool) {
            ...
            if (itemName.equals("Pot") && !((String)this.potionChoose.getValue()).equalsIgnoreCase("first") && !NewOffHand.mc.player.inventory.getStackInSlot(i).stackTagCompound.toString().split(":")[2].contains((CharSequence)this.potionChoose.getValue())) {
                return -1;
            }
            return i;
        }
    ...
    private void toOffHand(final int t) {
        if (!NewOffHand.mc.player.getHeldItemOffhand().isEmpty()) {
            if (this.firstChange) {
                this.prevSlot = t;
            }
        ...
        NewOffHand.mc.playerController.windowClick(0, (t < 9) ? (t + 36) : t, 0, ClickType.PICKUP, (EntityPlayer)NewOffHand.mc.player);
    ...
    public String getHudInfo() {
        if (((String)this.HudMode.getValue()).equals("Totem")) {
            ...
            if (NewOffHand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) {
                ++this.counts;
            }

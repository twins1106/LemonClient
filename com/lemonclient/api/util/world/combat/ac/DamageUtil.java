...
public class DamageUtil
{
    ...
    public static float calculateDamageThreaded(final double posX, final double posY, final double posZ, final PlayerInfo playerInfo, final boolean ignoreTerrain) {
        float finalDamage = 1.0f;
        try {
            final float doubleExplosionSize = 12.0f;
            final double distancedSize = playerInfo.entity.getDistance(posX, posY, posZ) / doubleExplosionSize;
            final double blockDensity = ignoreTerrain ? ignoreTerrainDecntiy(new Vec3d(posX, posY, posZ), playerInfo.entity.getEntityBoundingBox(), playerInfo.entity.world) : ((double)playerInfo.entity.world.getBlockDensity(new Vec3d(posX, posY, posZ), playerInfo.entity.getEntityBoundingBox()));
            final double v = (1.0 - distancedSize) * blockDensity;
            final float damage = (float)(int)((v * v + v) / 2.0 * 7.0 * doubleExplosionSize + 1.0);
    ...
    public static float getBlastReductionThreaded(final PlayerInfo playerInfo, float damage) {
        ...
        final float f = MathHelper.clamp((float)playerInfo.enchantModifier, 0.0f, 20.0f);
        damage *= 1.0f - f / 25.0f;
